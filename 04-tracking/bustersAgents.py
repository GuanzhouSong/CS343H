...
class GreedyBustersAgent(BustersAgent):
	...
	for i in range(1,len(livingGhosts)):
	    if livingGhosts[i]:
		pos=livingGhostPositionDistributions[index].argMax()
		index+=1
	...
	optDis=float('inf')
	    ...
	    actionDistance=self.distancer.getDistance(minPos, successorPosition)
            if actionDistance<optDis:
		optDis=actionDistance
	        optAction=action
	return optAction
