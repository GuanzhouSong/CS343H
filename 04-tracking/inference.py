...
class ParticleFilter(InferenceModule):
	...
	for p in self.legalPositions:
	    threshold=int(distribution[p]*self.numParticles)
	...
	if noisyDistance==None:
	    ...
	    for p in self.legalPositions:
	        realDistance=util.manhattanDistance(p,pacmanPosition)
	        prob=emissionModel[realDistance]
	        if prob>0:
		   # print p,"has prob larger than 0"
		    #print self.beliefs
	...
	if allPossible.totalCount() ==0.0:
	    ...
	    tmpParticles=[]
 	    for i in range(0,self.numParticles):
	        tmpParticles.append(util.sample(allPossible))
	...
	particlePositions=self.particles
	belifDistribution=util.Counter()
	for position in particlePositions:
	    belifDistribution[position]+=1
	belifDistribution.normalize()
	return belifDistribution
        #util.raiseNotDefined()
...
class JointParticleFilter:
	...
	#distribution.normalize()
	#for particle in positions:
	#    threshold=int(self.numParticles*distribution[particle])
	 #   for i in range(0,threshold):
	#	self.particles.append(particle)
#	self.beliefs=self.getBeliefDistribution()
	...
	for index, particle in enumerate(self.particles):
	    ...
	    for i in range(self.numGhosts):
		    ...
		    realDistance=util.manhattanDistance(pacmanPosition,ghostPosition)
		    prob=emissionModel[realDistance]
		    weight*=prob  #subject to change
	...
	if allPossible.totalCount()==0:
	    ...
	    for i in range(self.numGhosts):
		if(noisyDistances[i]==None):
		    print "change particle for ghost",i
		    for index,particle in enumerate(self.particles):
	...
	#	        particle=self.getParticleWithGhostInJail(particle,i)
	   ...
	   # print "all count not 0"
	    allPossible.normalize()
	    items=sorted(allPossible.items())
	    distribution=[i[1] for i in items]
	    values=[i[0] for i in items]
	    tempParticles=util.nSample(distribution,values,self.numParticles)
	    #print "This is called"
	    #for i in range(0,self.numParticles):
		#print i
