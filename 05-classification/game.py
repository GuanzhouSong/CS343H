...
class Grid:
    ...
    def __init__(self, width, height, initialValue=False,
        ...
        if initialValue not in [False, True]: raise Exception(
            'Grids can only contain booleans')
        self.CELLS_PER_INT = 30

        self.width = width
        self.height = height
        self.data = [[initialValue for y in range(height)] for x in
                     range(width)]
...
class GameStateData:
    ...
    def __hash__(self):
        ...
        return int((hash(tuple(self.agentStates)) + 13 * hash(
            self.food) + 113 * hash(tuple(self.capsules)) + 7 * hash(
            self.score)) % 1048575)
            ...
            if self.catchExceptions:
                try:
                    ...
                    if self.totalAgentTimes[
                        agentIndex] > self.rules.getMaxTotalTime(agentIndex):
