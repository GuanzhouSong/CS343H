...
class PacmanGraphics:
    ...
    def update(self, newState):
        ...
        if self.agentImages[agentIndex][
            0].isPacman != agentState.isPacman: self.swapImages(agentIndex,
                                                                agentState)
        prevState, prevImage = self.agentImages[agentIndex]
    ...
    def animatePacman(self, pacman, prevPacman, image):
        if self.frameTime < 0:
            print
            'Press any key to step forward, "q" to play'
            keys = wait_for_keys()
            if 'q' in keys:
                self.frameTime = 0.1
        if self.frameTime > 0.01 or self.frameTime < 0:
            ...
            self.movePacman(self.getPosition(pacman), self.getDirection(pacman),
                            image)
        refresh()
    ...
    def drawGhost(self, ghost, agentIndex):
        ...
        if dir == 'West':
            dx = -0.2
        leftEye = circle((screen_x + self.gridSize * GHOST_SIZE * (
                -0.3 + dx / 1.5), screen_y - self.gridSize * GHOST_SIZE * (
                                  0.3 - dy / 1.5)),
                         self.gridSize * GHOST_SIZE * 0.2, WHITE, WHITE)
        rightEye = circle((screen_x + self.gridSize * GHOST_SIZE * (
                0.3 + dx / 1.5), screen_y - self.gridSize * GHOST_SIZE * (
                                   0.3 - dy / 1.5)),
                          self.gridSize * GHOST_SIZE * 0.2, WHITE, WHITE)
        ...
        rightPupil = circle((screen_x + self.gridSize * GHOST_SIZE * (0.3 + dx),
                             screen_y - self.gridSize * GHOST_SIZE * (
                                     0.3 - dy)),
                            self.gridSize * GHOST_SIZE * 0.08, BLACK, BLACK)
        ghostImageParts = []
    ...
    def drawWalls(self, wallMatrix):
        ...
        for xNum, x in enumerate(wallMatrix):
            if self.capture and (xNum * 2) < wallMatrix.width: wallColor = \
            TEAM_COLORS[0]
            if self.capture and (xNum * 2) >= wallMatrix.width: wallColor = \
            TEAM_COLORS[1]
    ...
    def drawFood(self, foodMatrix):
        ...
        for xNum, x in enumerate(foodMatrix):
            if self.capture and (xNum * 2) <= foodMatrix.width: color = \
            TEAM_COLORS[0]
            if self.capture and (xNum * 2) > foodMatrix.width: color = \
            TEAM_COLORS[1]
            imageRow = []
        ...
        for x in range(len(self.distributionImages)):
            for y in range(len(self.distributionImages[0])):
                ...
                for weight, gcolor in zip(weights, colors):
                    color = [min(1.0, c + 0.95 * g * weight ** .3) for c, g in
                             zip(color, gcolor)]
...
def saveFrame():
    ...
    if not os.path.exists(POSTSCRIPT_OUTPUT_DIR): os.mkdir(
