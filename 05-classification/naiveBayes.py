...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
    ...
    def train(self, trainingData, trainingLabels, validationData,
        ...
        self.features = trainingData[
            0].keys()  # this could be useful for your code later...
        ...
        self.trainAndTune(trainingData, trainingLabels, validationData,
    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData,
        ...
        for k in kgrid:
            ...
            for pixel, label in denominator:
                condDis[(pixel, label, 0)] = float(
                    numerator[(pixel, label, 0)] + k) / float(
                    denominator[(pixel, label)] + k)
                condDis[(pixel, label, 1)] = float(
                    numerator[(pixel, label, 1)] + k) / float(
                    denominator[(pixel, label)] + k)
    ...
    def calculateLogJointProbabilities(self, datum):
        ...
        for label in self.legalLabels:
            ...
            for pixel in datum:
                ...
                logProb[label] += math.log(
                    self.bestCondition[(pixel, label, value)])
        return logProb
        # util.raiseNotDefined()
    ...
    def findHighOddsFeatures(self, label1, label2):
        ...
        for pixel in self.features:
            featureVal[pixel] = float(
                self.bestCondition[(pixel, label1, 1)]) / float(
                self.bestCondition[(pixel, label2, 1)])
